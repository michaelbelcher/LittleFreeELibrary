...
def microsoft(environ,start_response):
    ...
    response_headers = [('Location','http://' + fully_qualified_domain_name + '/books/'),
...
def android(environ, start_response):
    ...
    if system_version[0:1] < '6':
        ...
        location = "http://" + fully_qualified_domain_name + '/books/'
    else:
        #set_204after(ip,20)
...
def android_splash(environ, start_response):
    en_txt={ 'message':"Click on the button to go to the little e-book library",\
            'btn1':"GO TO LIBRARY", \
            "FQDN": fully_qualified_domain_name + '/books/', \
            'doc_root':get_iiab_env("WWWROOT") }
    es_txt={ 'message':"Haga clic en el botón para ir a la página de inicio de Biblioteca Libros",\
            "FQDN": fully_qualified_domain_name + '/books/', \
            'btn1':"Biblioteca",'doc_root':get_iiab_env("WWWROOT")}
    txt = en_txt
    if lang == "en":
        txt = en_txt
    elif lang == "es":
...
def android_https(environ, start_response):
    en_txt={ 'message':"""Please ignore the SECURITY warning which appears after clicking the first button""",\
             ...
             'btn1':'Then click this to go to library',\
             "FQDN": fully_qualified_domain_name + '/books/', \
            'doc_root':get_iiab_env("WWWROOT") }
    es_txt={ 'message':"Haga clic en el botón para ir a la página de inicio de Biblioteca Libros",\
            "FQDN": fully_qualified_domain_name + '/books/', \
...
def mac_splash(environ,start_response):
    ...
    en_txt={ 'message': "Click on the button to go to the little e-book library",\
            'btn1': "GO TO LIBRARY PAGE",'success_token': 'Success',
            "FQDN": fully_qualified_domain_name + '/books/', \
            'doc_root':get_iiab_env("WWWROOT")}
    es_txt={ 'message':"Haga clic en el botón para ir a la página de inicio de Biblioteca Libros",\
            "FQDN": fully_qualified_domain_name + '/books/', \
